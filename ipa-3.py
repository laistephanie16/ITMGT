...
def eta(first_stop, second_stop, route_map):
    ...
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    nicer_route_map = []
    for pair, value in route_map.items(): #change the format into a list of tuple 
        nicer_route_map.append( ( pair[0], pair[1], value['travel_time_mins'] ) )
    first_leg = next(leg for leg in nicer_route_map if leg[0]==first_stop)
    next_stop = first_leg[1]
    travel_time = first_leg[2]
    
    while next_stop!=second_stop:
        leg = next(leg for leg in nicer_route_map if leg[0]==next_stop)
        travel_time += leg[2]
        next_stop = leg[1]
